...
module.exports = class ElementStateProvider {
 ...
 /**
   * Wait for CSS property of element to reach certain value
   * @param {string} property Name of CSS property
   * @param {number} value Property value
   * @param {object} options Options for waiting
   * @returns {Promise<boolean>} true if CSS property reached value in time, false otherwise
   */
  async waitForCSSProperty (property, value,
    { timeout, interval, reverse } = {
      timeout: timeouts.explicit,
      interval: timeouts.polling,
      reverse: false,
    ...
    const func = async () =>
      await (await $(this.locator)).waitUntil(async function () {
        return (await this.getCSSProperty(property)).value === value;
     });
    return this._waitFor(
      func,
      { timeout, interval, reverse },
