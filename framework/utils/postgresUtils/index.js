...
class PostgresUtil {
  ...
  /**
   * Initialize connection with DB, send the query and close the connection with DB
   * @param {object} config Config
   * @param {string} query Query
   * @returns {Promise<void>} result
   */
  static async initExecuteAndClose(config, query) {
    await this.initConnection(config);
    const result = await this.query(config.dbName, query);
    this.close();
    return result;
  }
}
