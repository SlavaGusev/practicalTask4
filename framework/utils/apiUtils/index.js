...
class APIUtils {
  ...
  /**
   * Post request for authorization. Without logging the request body.
   * @param {string} url URL that will be used for the request
   * @param {Object} config config options for making requests
   * @returns {Object} API Response
   */
  async postRequestForAuthorization(url, data = {}, config = {}) {
    const fullURL = `${this.baseURL}${url}`;
    try {
      const response = await this.instance.post(url, data, config);
      Logger.info(
        `Executing POST request for authorization to ${fullURL} succeeded with status code ${response.status}`
      );
      return response;
    } catch (error) {
      if (error.response === undefined) {
        Logger.error(
          `POST request for authorization to ${fullURL} failed with error '${error.message}'`
        );
        throw error;
      } else {
        Logger.info(
          `Executing POST request for authorization to ${fullURL} succeeded with status code ${error.response.status}`
        );
        return error.response;
      }
    }
